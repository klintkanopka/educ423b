...
| Hey! Overall really nice job. I will say, something weird happened and it was hard (impossible eventually) for me to run your code natively in the document you submitted - so be sure to test it in the `.qmd` before you submit. Check my inline comments for a little bit of feedback - but there wasn't much to say!
| Grade: 5/5
...
> Overall, this totally works! I will make three notes, though. First, sometimes different packages can do weird things with factors that you don't expect. For that reason, I tend to prefer to store variables as character strings and then convert them to factors if I need them for that specifically.
>
> Second, you also could have done the recoding of the categorical variables with a mutate() and case_when(). With gender, it would have looked like:
>
> `d <- d |>`
>
...
> `gender == 2 ~ 'female'))`
>
> For the animal fighting cases, I'd take a look at what TRUE does inside of a `case_when()`, too
>
> Third, `%>%` and `|>` seem to do mostly the same thing, but there are some subtle differences between them! I've come to prefer `|>` because it requires you to put the parenthesis after a function name:
>
> `d %>% na.omit` works!
>
> `d %>% na.omit()` works!
...
> `d |> na.omit()` does!
>
> And, because *technically* `|>` is slightly faster.
...
2.  **Is there a relationship between confidence in winning fights and respondent age?**
    ...
    > I really like the approach of age binning here!
