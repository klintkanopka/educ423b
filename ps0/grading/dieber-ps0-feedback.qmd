...
> Hi Jürgen! Great job - I made a few *small* notes below, but there isn't much to critique/comment on. Check the official solutions for potentially more efficient ways to do some of this stuff that may be helpful!
>
> Grade: 5/5
...
2.  Create new, dichotomized versions of the `animalfight1_XX` variables. Code them so that the variable takes a value of `1` if the respondent believes they can beat the animal in a fight and `0` otherwise. Name these new variables in the format: `d_animalname`
    ...
    > Nice base `R` solution. Totally works, totally reasonable - but you might find a solution that uses `mutate()` and `case_when()` a little more elegant/easier - and require less copy/paste. Check the official solutions for that.
...
# i had to rearrange some code so this block would run - please check for errors in the future. Also, I asked you to rank animals strongst to weakest, but you gave weakest to strongest. 

# also note that in quarto documents, you don't need to call "print()" on objects you want to display
...
# let's show the correct way:
arrange(d_sorted, probability)
